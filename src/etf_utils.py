...
def compute_etf_field(image, iterations=5, kernel_size=5):
    global flowField, refinedETF, gradientMag
    h, w = image.shape
    SIZE = (h, w, 3)
    ...
    src_n = cv2.normalize(image.astype('float32'), None, 0.0, 1.0, cv2.NORM_MINMAX)
    ...
    for i in range(h):
        for j in range(w):
            ...
            flowField[i][j] = n / norm if norm != 0 else n

    rotateFlow(flowField, flowField, 90.0)

    for _ in range(iterations):
        refine_ETF(kernel_size)

    # Extraer solo los dos primeros canales (x, y)
    return flowField[:, :, :2]
